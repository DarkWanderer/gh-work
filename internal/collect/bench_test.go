...
// three sources, through the fake GraphQL client) over a synthetic org with
// 1000 open PRs (GitHub search's own result cap) spread across 20 repos,
// each contributing one unresolved review thread and one failing check,
// plus one failing default-branch check per repo.
func BenchmarkCollectAndRender(b *testing.B) {
	...
	srcs := []source.Source{reviewthreads.New(), prchecks.New(), branchchecks.New()}
